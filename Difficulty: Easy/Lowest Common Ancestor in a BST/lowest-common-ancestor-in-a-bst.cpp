...
class Solution {
  ...
  Node * ans = new Node(-1);
    ...
    bool helper(Node * node, Node* n1, Node* n2){
        if(node == nullptr) return false;
        bool left = helper(node->left, n1, n2);
        bool right = helper(node->right, n1,n2);
        if(left && right){
            ans = node;
            return false;
        }
        else if((left || right) && (node->data == n1->data || node->data==n2->data)) {
            ans = node;
            return true;
        }
        if(node->data == n1->data || node->data==n2->data) return true;
        return left||right;
    }
};
